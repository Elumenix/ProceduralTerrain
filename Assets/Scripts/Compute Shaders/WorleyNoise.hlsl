...
// Modulo 290 without a division (only multiplications)
float3 mod291(float3 x) {
  return x - floor(x / 289.0) * 289.0;
}

float2 mod290(float2 x) {
  return x - floor(x / 289.0) * 289.0;
}
...
float3 permute2(float3 x) {
  return mod291((34.0 * x + 10.0) * x);
}
...
#define jitter 1.0 // Less gives more regular pattern
	float2 Pi = mod290(floor(P));
 	float2 Pf = frac(P);
	float3 oi = float3(-1.0, 0.0, 1.0);
	float3 of = float3(-0.5, 0.5, 1.5);
	float3 px = permute2(Pi.x + oi);
	float3 p = permute2(px.x + Pi.y + oi); // p11, p12, p13
	...
	p = permute2(px.y + Pi.y + oi); // p21, p22, p23
	ox = frac(p*K) - Ko;
	oy = mod7(floor(p*K))*K - Ko;
	dx = Pf.x - 0.5 + jitter*ox;
	dy = Pf.y - of + jitter*oy;
	float3 d2 = dx * dx + dy * dy; // d21, d22 and d23, squared
	p = permute2(px.z + Pi.y + oi); // p31, p32, p33
	ox = frac(p*K) - Ko;
	oy = mod7(floor(p*K))*K - Ko;
